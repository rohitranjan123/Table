...
const TOTAL_COLS = 200; // 0 to 199 for data, 200 is for Actions
const OVERSCAN = 2; 
...
export default function App() {
  ...
  const populateDOMHeader = useCallback((headerNode, actualColId) => {
      // FIX: Cast actualColId to String and use Strict Equality (===) to fix the `'' == 0` JS bug
      ...
      headerNode.innerHTML = `<div class="flex items-center justify-between w-full pointer-events-none group"><span class="truncate">${title}</span>${sortIconHtml}</div>`;
      headerNode.className = `absolute top-0 h-full px-4 py-3 text-sm font-semibold tracking-wider border-b select-none transition-colors border-r border-gray-600 bg-gray-900 ${actualColId === TOTAL_COLS ? 'text-center text-indigo-300' : isSortable ? 'cursor-pointer hover:bg-gray-700' : ''} ${actualColId === 0 ? 'text-white border-gray-500' : 'text-gray-200'}`;
  }, []);

  // --- THE CORE VANILLA REDRAW ENGINE ---
  const redrawVanillaDOM = useCallback((scrollTop, scrollLeft) => {
    ...
    if (bodyCenterWrapperRef.current && headerCenterWrapperRef.current) {
        bodyCenterWrapperRef.current.style.transform = `translate3d(-${scrollLeft % COL_WIDTH}px, -${scrollTop % ROW_HEIGHT}px, 0)`;
        headerCenterWrapperRef.current.style.transform = `translate3d(-${scrollLeft % COL_WIDTH}px, 0px, 0)`;
    }
    if (bodyLeftWrapperRef.current) bodyLeftWrapperRef.current.style.transform = `translate3d(0, -${scrollTop % ROW_HEIGHT}px, 0)`;
    if (bodyRightWrapperRef.current) bodyRightWrapperRef.current.style.transform = `translate3d(0, -${scrollTop % ROW_HEIGHT}px, 0)`;
    ...
    for (let r = 0; r < rowPoolSize; r++) {
      ...
      const rowYTransform = `translate3d(0, ${offsetRow * ROW_HEIGHT}px, 0)`;
      ...
      if (rowCenterRefs.current[r]) {
          rowCenterRefs.current[r].style.transform = rowYTransform;
          rowCenterRefs.current[r].style.display = isVisibleRow ? 'flex' : 'none';
          rowCenterRefs.current[r].className = rowBaseClass;
      }
      if (rowLeftRefs.current[r]) {
          rowLeftRefs.current[r].style.transform = rowYTransform;
          rowLeftRefs.current[r].style.display = isVisibleRow ? 'flex' : 'none';
          rowLeftRefs.current[r].className = rowBaseClass;
      }
      if (rowRightRefs.current[r]) {
          rowRightRefs.current[r].style.transform = rowYTransform;
          rowRightRefs.current[r].style.display = isVisibleRow ? 'flex' : 'none';
          rowRightRefs.current[r].className = rowBaseClass;
      }
      ...
      for (let c = 0; c < leftCols.length; c++) {
          ...
          if (!cellNode) continue;
          cellNode.style.transform = `translate3d(${c * COL_WIDTH}px, 0, 0)`;
    ...
    for (let c = 0; c < colPoolSize; c++) {
        ...
        headerNode.style.display = logicalColIndex < centerCols.length ? 'flex' : 'none';
    ...
    for (let c = 0; c < leftCols.length; c++) {
        ...
        if (headerNode) {
            headerNode.style.transform = `translate3d(${c * COL_WIDTH}px, 0, 0)`;
  ...
  useEffect(() => {
      // FIX: Instead of empty strings (which coerce to `0` in JS), we explicitly use an 'INVALID' string.
      // This completely resolves the coercion bug where Column 0 (ID) refused to paint.
  ...
  }, [isPinned, rowPoolSize, colPoolSize, redrawVanillaDOM]);
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
        if (!isScrolling) {
            isScrolling = true;
            setActiveCell(null); 
            if (bodyCenterWrapperRef.current) bodyCenterWrapperRef.current.style.pointerEvents = 'none';
        }
        clearTimeout(scrollTimeout);
        scrollTimeout = setTimeout(() => {
            isScrolling = false;
            if (bodyCenterWrapperRef.current) bodyCenterWrapperRef.current.style.pointerEvents = 'auto';
        }, 150);
  ...
  const handleTableClick = (e) => {
      ...
      if (type === 'action') {
          ...
          if (actionBtn) {
              ...
              msg.innerText = `${actionType.toUpperCase()} triggered for Record ID: ${rowId}`;
  ...
  const handleEditorCommit = (value) => {
      ...
      const paneRefs = [cellCenterRefs.current, cellLeftRefs.current, cellRightRefs.current];
      paneRefs.forEach(pane => {
          const nodeKey = Object.keys(pane).find(key => pane[key]?.dataset?.coord === `${activeCell.rowId}-${activeCell.colIndex}`);
    ...
    <div className="h-screen w-screen bg-gray-100 flex flex-col p-6 font-sans box-border overflow-hidden">
      ...
      <div className={`flex-1 bg-white rounded-xl shadow-xl border border-gray-300 flex flex-col overflow-hidden relative transition-opacity ${isPending ? 'opacity-70' : 'opacity-100'}`}>
        ...
        <div ref={scrollContainerRef} className="overflow-auto relative bg-white flex-1 select-none">
          ...
          <div style={{ width: `${(TOTAL_COLS + 1) * COL_WIDTH}px`, height: `${(TOTAL_ROWS * ROW_HEIGHT) + 48}px`, position: 'absolute', top: 0, left: 0 }} />

          <div className="sticky top-0 left-0 overflow-hidden" style={{ width: '100%', height: '100%', zIndex: 10 }}>
             
             {/* --- HEADERS --- */}
             <div className="absolute top-0 left-0 right-0 h-[48px] z-20 shadow-md bg-gray-900" onClick={handleHeaderClick}>
               
               <div className={`absolute left-0 top-0 h-full bg-gray-900 z-30 transition-shadow ${isPinned ? 'shadow-[4px_0_10px_rgba(0,0,0,0.5)] border-r-2 border-indigo-500' : ''}`} style={{ width: leftCols.length * COL_WIDTH, display: isPinned ? 'block' : 'none' }}>
                   {leftCols.map((_, i) => <div key={i} ref={el => headerLeftRefs.current[i] = el} className="hidden" style={{ width: `${COL_WIDTH}px`, height: '100%' }} />)}
               </div>
               
               <div className="absolute top-0 h-full bg-gray-900 overflow-hidden" style={{ left: leftCols.length * COL_WIDTH, right: rightCols.length * COL_WIDTH }}>
                   <div ref={headerCenterWrapperRef} className="relative w-full h-full" style={{ willChange: 'transform' }}>
                       {Array.from({ length: colPoolSize }).map((_, c) => <div key={c} ref={el => headerCenterRefs.current[c] = el} className="hidden" style={{ width: `${COL_WIDTH}px`, height: '100%' }} />)}
                   </div>
               </div>

               <div className={`absolute right-0 top-0 h-full bg-gray-900 z-30 transition-shadow ${isPinned ? 'shadow-[-4px_0_10px_rgba(0,0,0,0.5)] border-l-2 border-indigo-500' : ''}`} style={{ width: rightCols.length * COL_WIDTH, display: isPinned ? 'block' : 'none' }}>
                   {rightCols.map((_, i) => <div key={i} ref={el => headerRightRefs.current[i] = el} className="hidden" style={{ width: `${COL_WIDTH}px`, height: '100%' }} />)}
               </div>

             </div>

             {/* --- BODY --- */}
             <div className="absolute top-[48px] left-0 right-0 bottom-0 z-10" onClick={handleTableClick}>
               
               <div className={`absolute left-0 top-0 h-full bg-white z-30 transition-shadow ${isPinned ? 'shadow-[4px_0_10px_rgba(0,0,0,0.05)] border-r-2 border-indigo-500' : ''}`} style={{ width: leftCols.length * COL_WIDTH, display: isPinned ? 'block' : 'none' }}>
                   <div ref={bodyLeftWrapperRef} className="relative w-full h-full" style={{ willChange: 'transform' }}>
                       {Array.from({ length: rowPoolSize }).map((_, r) => (
                           <div key={r} ref={el => rowLeftRefs.current[r] = el} className="hidden h-[44px]">
                               {leftCols.map((_, c) => <div key={`${r}-${c}`} ref={el => cellLeftRefs.current[`${r}-${c}`] = el} className="hidden" style={{ width: `${COL_WIDTH}px` }} />)}
                           </div>
                       ))}
                   </div>
               </div>

               <div className="absolute top-0 h-full bg-white overflow-hidden" style={{ left: leftCols.length * COL_WIDTH, right: rightCols.length * COL_WIDTH }}>
                   <div ref={bodyCenterWrapperRef} className="relative w-full h-full" style={{ willChange: 'transform' }}>
                       {Array.from({ length: rowPoolSize }).map((_, r) => (
                           <div key={r} ref={el => rowCenterRefs.current[r] = el} className="hidden h-[44px]">
                               {Array.from({ length: colPoolSize }).map((_, c) => <div key={`${r}-${c}`} ref={el => cellCenterRefs.current[`${r}-${c}`] = el} className="hidden" style={{ width: `${COL_WIDTH}px` }} />)}
                           </div>
                       ))}
                   </div>
               </div>

               <div className={`absolute right-0 top-0 h-full bg-white z-30 transition-shadow ${isPinned ? 'shadow-[-4px_0_10px_rgba(0,0,0,0.05)] border-l-2 border-indigo-500' : ''}`} style={{ width: rightCols.length * COL_WIDTH, display: isPinned ? 'block' : 'none' }}>
                   <div ref={bodyRightWrapperRef} className="relative w-full h-full" style={{ willChange: 'transform' }}>
                       {Array.from({ length: rowPoolSize }).map((_, r) => (
                           <div key={r} ref={el => rowRightRefs.current[r] = el} className="hidden h-[44px]">
                               {rightCols.map((_, c) => <div key={`${r}-${c}`} ref={el => cellRightRefs.current[`${r}-${c}`] = el} className="hidden" style={{ width: `${COL_WIDTH}px` }} />)}
                           </div>
                       ))}
                   </div>
               </div>
        ...
        <div className="bg-gray-50 border-t border-gray-200 p-3 text-xs text-gray-500 flex justify-between z-30 shrink-0">
          ...
          <span>Strategy: 3-Pane DOM Freezing (Independent X-Translation Layers)</span>
        </div>
      </div>
    </div>
  );
