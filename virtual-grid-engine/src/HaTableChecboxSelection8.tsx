...
const getInitialWidth = (col) => {
    if (col === 0) return 60; // Row Selection Checkbox
    if (col === TOTAL_COLS) return 150; // Actions
    return DEFAULT_COL_WIDTH;
};

// --- MOCK DATA GENERATORS ---
const getColumnType = (colIndex) => {
  if (colIndex === 0) return 'row-select'; // NEW: Dedicated selection column
...
const getCellData = (rowIndex, colIndex, edits, selectedSet) => {
  ...
  if (colIndex === 0) return selectedSet.current.has(rowIndex); // Real-time Boolean
...
const generateCellHTML = (type, value) => {
  if (type === 'row-select') {
      return `<div style="display:flex; justify-content:center; width:100%;"><input type="checkbox" ${value ? 'checked' : ''} style="width: 16px; height: 16px; accent-color: #4f46e5; pointer-events: none;" /></div>`;
  }
  if (type === 'action') {
      return `
...
export default function App() {
  ...
  const { leftCols, rightCols, centerCols } = useMemo(() => {
    if (isPinned) {
        // Pin Checkbox, ID, and Role
        const left = visibleColOrder.filter(c => c === 0 || c === 1 || c === 5);
        const right = visibleColOrder.filter(c => c === TOTAL_COLS);
        const center = visibleColOrder.filter(c => c !== 0 && c !== 1 && c !== 5 && c !== TOTAL_COLS);
  ...
  const populateDOMCell = useCallback((cellNode, actualRowId, actualColId) => {
      ...
      // CACHE OPTIMIZATION: Tie the specific boolean selection state strictly into the cell's cache key!
      // This ensures that when a row's selection changes, ONLY the exact checkbox cell natively repaints itself.
      ...
      setClass(cellNode, `ag-cell absolute top-0 flex items-center px-3 overflow-hidden text-sm whitespace-nowrap border-r border-gray-200 h-full transition-none ${actualColId === 1 ? 'font-bold bg-gray-100/90 text-gray-900 border-gray-300' : 'text-gray-700'} ${['editable-text', 'dropdown', 'checkbox', 'row-select'].includes(type) ? 'cursor-pointer hover:bg-black/5' : ''}`);
  }, []);

  const populateDOMHeader = useCallback((headerNode, actualColId) => {
      const colType = getColumnType(actualColId);
      
      // DYNAMIC HEADER CACHING: Forces "Select All" header to repaint if row counts change
      const headerStateStr = colType === 'row-select' ? `${actualColId}-${selectedRowsRef.current.size}` : String(actualColId);
      ...
      if (colType === 'row-select') {
          const isAll = selectedRowsRef.current.size === TOTAL_ROWS;
          ...
          headerContentHTML = `<div class="flex items-center justify-between w-full h-full pointer-events-none group pr-3"><span class="truncate">${getColTitle(actualColId)}</span>${sortIcon}</div>`;
      }
  ...
  const redrawVanillaDOM = useCallback((scrollTop, scrollLeft) => {
    ...
    for (let r = 0; r < rowPoolSize; r++) {
      ...
      const isSelected = selectedRowsRef.current.has(actualRowId);
      const rowYTransform = `translate3d(0, ${logicalRowIndex * ROW_HEIGHT}px, 0)`;
      const bgClass = isSelected ? 'bg-indigo-50/80 border-indigo-200 hover:bg-indigo-100' : (logicalRowIndex % 2 === 0 ? 'bg-white border-gray-200' : 'bg-gray-50/50 border-gray-200');
      const rowClass = `absolute left-0 flex border-b transition-none hover:bg-blue-50/50 w-full h-[${ROW_HEIGHT}px] ${bgClass}`;
  ...
  const handleTableClick = useCallback((e) => {
      ...
      if (headerNode && headerNode.dataset.col) {
          ...
          // 2. HEADER SELECT-ALL LOGIC
          if (colType === 'row-select') {
              if (selectedRowsRef.current.size === TOTAL_ROWS) selectedRowsRef.current.clear();
              else {
                  for (let i = 0; i < TOTAL_ROWS; i++) selectedRowsRef.current.add(i);
              }
              redrawVanillaDOM(scrollContainerRef.current.scrollTop, scrollContainerRef.current.scrollLeft);
              setRenderTick(t => t + 1);
              return;
    ...
    <div className="h-screen w-screen bg-gray-100 flex flex-col p-6 font-sans box-border overflow-hidden">
      ...
      <div className="w-full flex-shrink-0 mb-4 flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-300">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm text-gray-500 ml-4 border-l-2 pl-4 border-gray-300">
             Try clicking the first column checkbox, or the global header checkbox, to batch-select rows.
          </span>
        </div>
      ...
      <div className={`flex-1 bg-white rounded-xl shadow-xl border border-gray-300 flex flex-col overflow-hidden relative transition-opacity ${isPending ? 'opacity-70' : 'opacity-100'}`}>
        ...
        <div className="bg-gray-50 border-t border-gray-200 p-3 text-xs text-gray-500 flex justify-between z-30 shrink-0">
          ...
          <span className="font-bold text-indigo-600 bg-indigo-100 px-3 py-1 rounded-full">Selected Records: {selectedRowsRef.current.size.toLocaleString()}</span>
        </div>
      </div>
    </div>
  );
