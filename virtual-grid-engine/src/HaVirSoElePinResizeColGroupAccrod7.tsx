...
// --- CONFIGURATION ---
const ROW_HEIGHT = 44; 
const HEADER_HEIGHT = 60; // 2-Tier Hierarchy Height
...
const getColTitle = (colIndex) => {
  ...
  if (titles[colIndex]) return titles[colIndex];
...
const generateCellHTML = (type, value) => {
  if (type === 'action') {
      ...
      <div style="display: flex; gap: 14px; justify-content: center; width: 100%; font-size: 16px;">
          <span data-action="preview" title="Preview" style="cursor: pointer; transition: transform 0.1s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'">👁️</span>
          <span data-action="save" title="Save" style="cursor: pointer; transition: transform 0.1s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'">💾</span>
          <span data-action="delete" title="Delete" style="cursor: pointer; transition: transform 0.1s; filter: grayscale(100%);" onmouseover="this.style.transform='scale(1.2)'; this.style.filter='none';" onmouseout="this.style.transform='scale(1)'; this.style.filter='grayscale(100%)';">🗑️</span>
      </div>`;
  }
  ...
  if (type === 'tags') {
    const tagsHtml = value.map(t => `<span style="background: #1e293b; color: #93c5fd; border: 1px solid #3b82f6; border-radius: 4px; padding: 2px 6px; font-size: 10px; font-weight: bold; text-transform: uppercase;">${t}</span>`).join('');
...
const setStyle = (node, prop, value) => {
    if (!node) return;
    if (node[`_${prop}`] !== value) { node.style[prop] = value; node[`_${prop}`] = value; }
};
const setClass = (node, value) => {
    if (!node) return;
    if (node._class !== value) { node.className = value; node._class = value; }
};
...
export default function App() {
  ...
  // 1. THE VISIBLE ARRAY FILTER
  // This engine physically filters out grouped columns if their parent group is collapsed
  const { visibleColOrder } = useMemo(() => {
    ...
    colOrder.forEach(c => {
        ...
        if (g && collapsedGroupsRef.current[g]) {
            // If collapsed, we only render the FIRST column of the group sequence
            if (!seen.has(g)) {
                visible.push(c);
                seen.add(g);
            }
  ...
  // 2. THE 3-PANE DISTRIBUTOR
  // Distributes the newly filtered visible columns across the Left, Right, and Center pinned layouts
  const { leftCols, rightCols, centerCols } = useMemo(() => {
    if (isPinned) {
        const left = visibleColOrder.filter(c => c === 0 || c === 4);
        const right = visibleColOrder.filter(c => c === TOTAL_COLS);
        const center = visibleColOrder.filter(c => c !== 0 && c !== 4 && c !== TOTAL_COLS);
        return { leftCols: left, rightCols: right, centerCols: center };
    } else {
  ...
  const buildLayout = useCallback((cols) => {
      ...
      for (let i = 0; i < cols.length; i++) {
          const c = cols[i];
          pos.push(total);
          const w = colWidthsRef.current[c] || DEFAULT_COL_WIDTH;
  ...
  const populateDOMCell = useCallback((cellNode, actualRowId, actualColId) => {
      ...
      cellNode.innerHTML = generateCellHTML(type, getCellData(actualRowId, actualColId, editsRef));
      setClass(cellNode, `ag-cell absolute top-0 flex items-center px-3 overflow-hidden text-sm whitespace-nowrap border-r border-gray-200 h-full transition-none ${actualColId === 0 ? 'font-bold bg-gray-100/90 text-gray-900 border-gray-300' : 'text-gray-700'} ${['editable-text', 'dropdown', 'checkbox'].includes(type) ? 'cursor-pointer hover:bg-black/5' : ''}`);
  }, []);
  ...
  const populateDOMHeader = useCallback((headerNode, actualColId) => {
      ...
      let sortIcon = '';
      if (ruleIndex >= 0) sortIcon = `<span class="flex items-center text-xs ml-1 text-blue-400">${sortRulesRef.current[ruleIndex].dir === 'asc' ? '▲' : '▼'}</span>`;
      else if (isSortable) sortIcon = `<span class="flex items-center text-xs ml-1 text-gray-500 opacity-30 group-hover:opacity-100 transition-opacity">↕</span>`;

      headerNode.innerHTML = `
          <div class="flex items-center justify-between w-full h-full pointer-events-none group pr-3">
              <span class="truncate">${getColTitle(actualColId)}</span>${sortIcon}
          </div>
          <div class="resize-handle hover:bg-blue-500 hover:opacity-100 opacity-0 transition-opacity" data-col="${actualColId}" style="position: absolute; right: 0; top: 0; width: 6px; height: 100%; cursor: col-resize; z-index: 100; pointer-events: auto;"></div>
      `;
      
      setStyle(headerNode, 'top', hasGroup ? '24px' : '0px');
      setStyle(headerNode, 'height', hasGroup ? '36px' : '60px');
      
      setClass(headerNode, `ag-header-cell absolute pl-3 text-[12px] font-semibold tracking-wide border-b select-none transition-none border-r border-gray-700 bg-slate-800 ${actualColId === TOTAL_COLS ? 'text-center text-blue-300' : isSortable ? 'cursor-grab hover:bg-slate-700' : ''} text-gray-200`);
  }, []);
  ...
  const redrawVanillaDOM = useCallback((scrollTop, scrollLeft) => {
    ...
    for (let r = 0; r < rowPoolSize; r++) {
      ...
      const rowYTransform = `translate3d(0, ${logicalRowIndex * ROW_HEIGHT}px, 0)`;
      const rowClass = `absolute left-0 flex border-b border-gray-200 transition-none ${logicalRowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-blue-50/50 w-full h-[${ROW_HEIGHT}px]`;
      ...
      [rowCenterRefs.current[r], rowLeftRefs.current[r], rowRightRefs.current[r]].forEach(rowNode => {
          if (rowNode) { setStyle(rowNode, 'transform', rowYTransform); setStyle(rowNode, 'display', rowDisplay); setClass(rowNode, rowClass); }
      });
    ...
    // 3. SPANNING GROUP HEADERS (Accordion Logic)
    const updateGroups = (layout, refs) => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if (g) {
                ...
                const newHtmlKey = `${g.name}-${isCollapsed}-${count}`; // Forces header repaint on collapse toggle
                ...
                if (node.dataset.name !== newHtmlKey) {
                    ...
                    node.innerHTML = `
                        <div class="w-full h-full flex items-center px-3 text-[12px] text-gray-300 font-bold border-b border-r border-gray-700 bg-slate-900 justify-between">
                            <div class="flex items-center collapse-trigger cursor-pointer hover:text-white transition-colors" data-group="${g.name}">
                                <span class="mr-2 text-[10px] bg-slate-700 hover:bg-slate-600 rounded px-1 leading-none py-[2px] flex items-center justify-center">${icon}</span>
                                <span>${g.name}</span>
                                ${extraHtml}
                            </div>
                        </div>
  ...
  // Ensure layout triggers physical Vanilla DOM refresh whenever Pinning or Accordion collapsing changes the arrays
  useEffect(() => {
      recalcLayouts();
  ...
  }, [isPinned, colOrder, collapsedGroupsTick, rowPoolSize, colPoolSize, recalcLayouts, updateShellDimensions, redrawVanillaDOM]);
  ...
  useEffect(() => {
      ...
      const handleMouseMove = (e) => {
          ...
          rafId = requestAnimationFrame(() => {
              ...
              recalcLayouts(); updateShellDimensions();
  ...
  const handleTableClick = useCallback((e) => {
      // --- NEW: ACCORDION COLLAPSE TRIGGER INTERCEPTION ---
      const collapseTrigger = e.target.closest('.collapse-trigger');
      if (collapseTrigger) {
          e.stopPropagation();
          const groupName = collapseTrigger.dataset.group;
      ...
      if (cellNode) {
          ...
          if (type === 'action') {
              ...
              if (actionBtn) {
                  ...
                  document.body.appendChild(msg); setTimeout(() => document.body.removeChild(msg), 2000);
              }
              return;
          }
          if (type === 'checkbox') {
          ...
          if (type === 'editable-text' || type === 'dropdown') {
              ...
              setActiveCell({ rowId, colIndex: colId, type, rect: { top: rect.top, left: rect.left, width: rect.width, height: rect.height } });
          }
      }
  ...
  // --- HTML5 DRAG & DROP SWAPPING ENGINE ---
  ...
  const handleDragOver = (e) => {
      ...
      if (headerNode) {
          ...
          headerNode.classList.add('border-l-4', 'border-blue-500');
      }
  };
  ...
  const handleDrop = (e) => {
      e.preventDefault();
      document.querySelectorAll('.ag-header-cell').forEach(el => el.classList.remove('border-l-4', 'border-blue-500'));
      const sourceCol = parseInt(e.dataTransfer.getData('text/plain'));
      const targetNode = e.target.closest('.ag-header-cell');
      ...
      if (targetNode && targetNode.dataset.col) {
          ...
          if (!isNaN(sourceCol) && sourceCol !== targetCol) {
              setColOrder(prev => {
                  const newOrder = [...prev];
                  const idxA = newOrder.indexOf(sourceCol); const idxB = newOrder.indexOf(targetCol);
  ...
  return (
    <div className="h-screen w-screen bg-gray-100 flex flex-col p-6 font-sans box-border overflow-hidden">
      <style>{`.is-resizing-col * { user-select: none !important; }`}</style>
      ...
      <div className="w-full flex-shrink-0 mb-4 flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-300">
        <div className="flex items-center gap-4">
          <button onClick={() => { isMultiSortRef.current = !isMultiSortRef.current; sortRulesRef.current = []; executeSort(); }} className={`px-4 py-2 rounded-lg font-semibold transition-colors ${isMultiSortRef.current ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
          ...
          <span className="text-sm text-gray-500 ml-4 border-l-2 pl-4 border-gray-300">
             Click <b>[➖]</b> to compress grouped columns into an accordion. Try Drag & Drop too!
          </span>
        </div>
      ...
      <div className={`flex-1 bg-white rounded-xl shadow-xl border border-gray-300 flex flex-col overflow-hidden relative transition-opacity ${isPending ? 'opacity-70' : 'opacity-100'}`}>
        ...
        <div ref={scrollContainerRef} className="overflow-auto relative bg-white flex-1 select-none">
          <div ref={scrollEnforcerRef} style={{ height: `${(TOTAL_ROWS * ROW_HEIGHT) + HEADER_HEIGHT}px`, position: 'absolute', top: 0, left: 0 }} />
          ...
          <div className="sticky top-0 left-0 overflow-hidden" style={{ width: '100%', height: '100%', zIndex: 10 }} onMouseDown={handleTableMouseDown} onClick={handleTableClick} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop} onDragEnd={handleDragEnd}>
             ...
             {/* --- HEADERS (AG Grid 2-Tier Architecture) --- */}
             ...
             <div className={`absolute top-[${HEADER_HEIGHT}px] left-0 right-0 bottom-0 z-10 transition-none`}>
               <div ref={leftPaneBodyRef} className={`absolute left-0 top-0 h-full bg-white z-30 transition-shadow transition-none ${isPinned ? 'shadow-[4px_0_10px_rgba(0,0,0,0.05)] border-r-2 border-blue-500' : ''}`} style={{ display: isPinned ? 'block' : 'none' }}>
               ...
               <div ref={rightPaneBodyRef} className={`absolute right-0 top-0 h-full bg-white z-30 transition-shadow transition-none ${isPinned ? 'shadow-[-4px_0_10px_rgba(0,0,0,0.05)] border-l-2 border-blue-500' : ''}`} style={{ display: isPinned ? 'block' : 'none' }}>
        ...
        <div className="bg-gray-50 border-t border-gray-200 p-3 text-xs text-gray-500 flex justify-between z-30 shrink-0">
          ...
          <span>Logical Filtering Engine (0ms Collapse/Expand Penalty)</span>
        </div>
      </div>
    </div>
  );
